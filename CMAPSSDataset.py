...
class CMAPSSDataset():
    def __init__(self, fd_number, batch_size, sequence_length,deleted_engine,feature_columns):
        ...
        # read train_FD00x
        data = pd.read_csv(os.path.join(data_root, f"train_FD00{fd_number}.txt"), delimiter="\s+", header=None)
        ...
        test_data = pd.read_csv(os.path.join(data_root, f"test_FD00{fd_number}.txt"), delimiter="\s+", header=None)
        test_data.columns = columns
        truth_data = pd.read_csv(os.path.join(data_root, f"RUL_FD00{fd_number}.txt"), delimiter="\s+", header=None)
