...
class KePINModel(keras.Model):
    ...
    def call(self, inputs, training=None):
        ...
        # Koopman operator
        koopman_out = self.koopman(z_seq, raw_inputs=inputs, training=training)
        ...
        if len(spec_feats.shape) == 1:
            ...
            # Already batched if condition_dim > 0
            # Just ensure it's padded/tiled if eigenvalues was subsampled
            # Wait, in KoopmanOperator.call, if batched, it tiled the subsampled eigenvalues to match batch_size.
            # So here spec_feats is already (batch_size, num_feats)
            spec_feats_batch = tf.cast(spec_feats, tf.float32)
    ...
    def get_koopman_matrix(self, inputs=None):
        """Return the learned Koopman operator matrix K.
        If condition_dim > 0, you must provide inputs (batch, T, features)
        to get the specific K(mu) for the batch.
        """
        if self.condition_dim > 0:
            if inputs is None:
                print("WARNING: condition_dim > 0 but inputs=None. Returning K(raw_inputs=None).")
                K, _ = self.koopman._get_K(raw_inputs=None)
                return K.numpy()
            else:
                K, _ = self.koopman._get_K(raw_inputs=inputs)
                return K.numpy()
        else:
            K, _ = self.koopman._get_K()
            return K.numpy()

    def get_eigenvalues(self, inputs=None):
        """Return the eigenvalues of the Koopman operator.
        If condition_dim > 0, provide inputs to get batch-specific eigenvalues.
        """
        if self.condition_dim > 0:
            K = self.get_koopman_matrix(inputs=inputs)
            if len(K.shape) == 3: # batched
