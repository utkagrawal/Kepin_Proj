...
DATASET_KEY = "CMAPSS_FD003"
CFG = CMAPSS_CONFIGS[DATASET_KEY]

def train_fd003_conditioned(output_dir: str, signal_type: str = 'a', verbose: int = 1, epochs: int = None, n_runs: int = None, start_run: int = 0):
    """Full 3-run optimized training for FD003 with Regime Conditioning."""
    ...
    print(f"\n{'='*70}")
    print(f"  KePIN FD003 Regime-Conditioned Training")
    ...
    print(f"\n{'='*70}")
    print(f"  FD003 REGIME-CONDITIONED ENSEMBLE ({CFG['n_runs']} runs):")
    ...
    with open(os.path.join(output_dir, "fd003_conditioned_results.json"), "w") as f:
...
def main():
    parser = argparse.ArgumentParser(description="KePIN FD003 Regime Conditioned")
    ...
    if args.output_dir is None:
        ...
        args.output_dir = os.path.join(_SCRIPT_DIR, f"results_fd003_signal_{args.signal_type.lower()}_{ts}")

    rmse = train_fd003_conditioned(
