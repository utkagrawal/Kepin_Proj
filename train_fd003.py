...
def train_fd003(output_dir: str, verbose: int = 1):
    """Full 3-run optimised training for FD003, returning ensemble RMSE."""
    ...
    print(f"\n{'='*70}")
    print(f"  KePIN FD003 Replication")
    ...
    print(f"\n{'='*70}")
    print(f"  FD003 ENSEMBLE ({CFG['n_runs']} runs):")
    ...
    with open(os.path.join(output_dir, "fd003_results.json"), "w") as f:
...
def main():
    parser = argparse.ArgumentParser(
        description="KePIN FD003 Replication -- target RMSE=11.42")
    ...
    if args.output_dir is None:
        ...
        args.output_dir = os.path.join(_SCRIPT_DIR, f"results_fd003_{ts}")

    rmse = train_fd003(args.output_dir, verbose=args.verbose)
