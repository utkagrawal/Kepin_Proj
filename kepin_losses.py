...
def spectral_stability_loss(koopman_outputs):
    """Enforce spectral radius ρ(K) ≤ ||K||_2 = max(σ) < 1.
    If 'sigma' is available (SVD mode), penalises max(σ) directly for stability
    and O(1) performance. Otherwise falls back to eigenvalue penalty.
    """
    if "sigma" in koopman_outputs:
        sigma = koopman_outputs["sigma"]
        sigma_max = tf.reduce_max(sigma, axis=-1)
        # Target maximum singular value = 0.99 for stability margin
        return tf.reduce_mean(tf.square(tf.nn.relu(sigma_max - 0.99)))
    
    # Fallback for full K mode
    eig_magnitudes = tf.abs(koopman_outputs["eigenvalues"])  # |λ_i|
...
def make_kepin_loss(loss_weights_layer: KePINLossWeights = None,
    ...
    The ``domain_mode`` controls which physics-informed losses are active:
      - ``"degradation"`` (default): all 7 losses (RUL, Koopman, spectral,
        monotonicity, multi-step, asymmetric, slope).
      - ``"forecasting"``: 4 losses (prediction, Koopman, spectral,
        multi-step). Monotonicity, asymmetric, and slope are disabled
        because they encode degradation-specific priors that are not
        applicable to weather or financial time series.
    ...
    Args:
        ...
        domain_mode:        ``"degradation"`` or ``"forecasting"``
    ...
    # Decide which losses are active based on domain
    if domain_mode == "forecasting":
        # For weather/finance: keep prediction + Koopman dynamics losses,
        # disable degradation-specific priors (monotonicity, asymmetric, slope)
    ...
    # Only count active losses for auto-weighting
    ...
    def loss_fn(y_true, y_pred, koopman_outputs):
        ...
        Args:
            y_true: (batch, 1) ground truth target
            y_pred: (batch, 1) predicted target
        ...
        l_spec = spectral_stability_loss(koopman_outputs)
