...
DATASET_KEY = "CMAPSS_FD004"
CFG = CMAPSS_CONFIGS[DATASET_KEY]

def train_fd004_conditioned(output_dir: str, signal_type: str = 'a', verbose: int = 1, epochs: int = None, n_runs: int = None, start_run: int = 0):
    """Full 3-run optimized training for FD004 with Regime Conditioning."""
    ...
    print(f"\n{'='*70}")
    print(f"  KePIN FD004 Regime Conditioned (Signal {signal_type.upper()})")
    print(f"  epochs={CFG['epochs']}, patience={CFG['patience']}, "
          f"lr={CFG['lr']}, batch={CFG['batch_size']}")
    print(f"{'='*70}")

    # Load Data and Signals
    data_path = os.path.join(_SCRIPT_DIR, "fd004_signals.npz")
    ...
    print(f"\n{'='*70}")
    print(f"  FD004 REGIME-CONDITIONED ENSEMBLE ({CFG['n_runs']} runs):")
    ...
    with open(os.path.join(output_dir, "fd004_conditioned_results.json"), "w") as f:
...
def main():
    parser = argparse.ArgumentParser(description="KePIN FD004 Regime Conditioned")
    ...
    if args.output_dir is None:
        ...
        args.output_dir = os.path.join(_SCRIPT_DIR, f"results_fd004_signal_{args.signal_type.lower()}_{ts}")

    rmse = train_fd004_conditioned(
